(() => {
  ...
  function thankYouStandings(data) {
    ...
    const rows = entries => entries.map(entry => `<div class="final-recap-thankyou-row">
      ...
      <span>${entry.score} ${word(entry.score,'pt','pts')} · ${entry.wins} ${word(entry.wins,'win','wins')}</span>
    </div>`).join('');
  ...
  function buildScreens(data) {
    ...
    const openingCopy = `Well guys, here it is. The final day of shoot.*<br><br>We’ve actually made it to end of this insane project, at least for us lucky Italians.<br><br>We’ve been through every season and every weather, every mood and every emotion, every Friday night shoot, every 12 hour day, every Helen speech, every 30 minutes martini. And we’ve made it through all of it thanks to one, beautiful, unifying thing: gambling.<br><br>I want to thank all of you for participating in this game and making it what it is now, because Totowrap doesn’t exist without a bunch of similarly mentally ill people coming together to fight over which second wrap was called at. So, from the bottom of my heart, thank you.<br><br>Not all of us have been here since the beginning and not all the people that started playing got to the finish line, but we were all part of it anyway and I want to share with you what we’ve accomplished in this incredible, crazy, amazing, nightmarish, infinite project we’ve been on. May it help us find the help we all so desperately need.<br><br>So we can win even more on the next one!<br><br><span class="final-recap-opening-footnote">*in Italy</span>`;
    ...
    const leastAccuracyCopy = data.leastAccurate
      ? `Average distance was ${esc(compactTime(data.leastAccurate.avgGap))} from the official wrap across ${data.leastAccurate.bets} ${word(data.leastAccurate.bets,'bet','bets')}.`
      : 'There is not enough completed data to calculate accuracy yet.';
