...
window.__TOTOSWRAP_DEBUG__ = {
  ...
  runBonusSimulation: async () => {
    ...
    try {
      ...
      return {
        candidates:
          getMultiUnitBonusCandidates('2099-01-01'),
        scoresBefore:
          JSON.parse(JSON.stringify(S.scores)),
        bonusesBefore:
          JSON.parse(
            JSON.stringify(S.multiUnitBonuses)
          )
      };
    } finally {
      S = original;
  ...
  try {
    ...
    const previousBonusRecord =
      getMultiUnitBonusRecord('2099-01-01');

    const before = {
      scores: JSON.parse(JSON.stringify(S.scores)),
      bonuses: JSON.parse(JSON.stringify(S.multiUnitBonuses)),
